...
from msgs.msg import SensorSync as SyncMsg, MTi200Data as Imu, DepthData as Depth
from geometry_msgs.msg import TwistStamped, Twist
from message_filters import Subscriber, ApproximateTimeSynchronizer
from scipy.spatial.transform import Rotation as R

ACCEPTABLE_SLOP = (
    0.01  # message arrival tolerance (s) -- NOTE: the IMU is running at 100 Hz.
)
...
class SyncFilter(Node):
    ...
    def __init__(self):
        ...
        self.R_dvl = np.array([[0, 1, 0], [1, 0, 0], [0, 0, 1]])
        self.R_flip_x = np.array([[-1, 0, 0], [0, 1, 0], [0, 0, 1]])
        ...
        self.R_invert_y = np.array([[1, 0, 0], [0, -1, 0], [0, 0, 1]])

        self.sync_publisher_ = self.create_publisher(
            SyncMsg, "/IDD_synchronized_data", 10
        )
        ...
        self.imu_only_sub = self.create_subscription(
            Imu, "/imu_synchronized_data", self.sync_callback_D, 10
        )
    ...
    def sync_callback_A(self, dvl_msg, imu_msg, depth_msg):
        ...
        imu_accel = np.array(
            [
                imu_msg.free_acceleration.x,
                imu_msg.free_acceleration.y,
                imu_msg.free_acceleration.z,
            ]
        )
        ...
        imu_gyro = np.array(
            [imu_msg.rate_of_turn.x, imu_msg.rate_of_turn.y, imu_msg.rate_of_turn.z]
        )
        ...
        imu_orientation = R.from_euler(
            "xyz", [imu_msg.orientation.x, imu_msg.orientation.y, imu_msg.orientation.z]
        )

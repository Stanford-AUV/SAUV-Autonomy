...
R_dvl = np.array([[0, 1, 0], [1, 0, 0], [0, 0, 1]])

# Combined transformation matrix for the IMU data
R_imu = np.array([[-1, 0, 0], [0, 1, 0], [0, 0, -1]])
...
def parse_data(extracted_data, topics):
    ...
    for topic in topics:
        ...
        if isinstance(
            extracted_data[topic][0], TwistStamped
        ):  # Example for TwistStamped
            linear = np.array(
                [
                    [msg.twist.linear.x, msg.twist.linear.y, msg.twist.linear.z]
                    for msg in extracted_data[topic]
                ]
            )
            ...
            extracted_data[topic][0], Vector3Stamped
        ):  # Example for Vector3Stamped
            vectors = np.array(
                [

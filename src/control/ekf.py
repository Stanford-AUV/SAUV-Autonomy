...
class EKF:
    ...
    def get_state(
        ...
        delta_t: float = (time - self._time).nanoseconds * 1e-9
    ...
    def handle_imu_measurement(
        ...
        """
        Handle an IMU measurement. The orientation is given as a 3-element array
        representing the roll, pitch, and yaw angles in radians. The linear acceleration
        is given as a 3-element array representing the acceleration in the x, y, and z
        directions in m/s^2. The covariance is a vector of length 6 representing the
        variances of the orientation and linear acceleration measurements.
        """
    ...
    def handle_dvl_measurement(
        self, linear_velocity: np.array, covariance: np.array, timestamp: Time
    ):
        self._predict(timestamp)
    ...
    def handle_depth_measurement(
        self, depth: np.array, covariance: np.array, timestamp: Time
    ):
        self._predict(timestamp)
    ...
    def _predict(self, timestamp: Time):
        if self._time is None:
            self._time = timestamp

        delta_t: float = (timestamp - self._time).nanoseconds * 1e-9
        self._time = timestamp

        self._state = self._extrapolate_state(delta_t)
        self._covariance = self._extrapolate_covariance(delta_t)

    def _update(self, innovation: np.array, covariance: np.array, fields: List[int]):
        H: np.array = self._get_H(fields)
    ...
    def _extrapolate_state(self, dt: float) -> np.array:
        ...
        state = F @ self._state
        state = self._wrap_angles(
            state, [StateIndex.YAW, StateIndex.PITCH, StateIndex.ROLL]
        )

        return state

    def _wrap_angles(self, state: np.array, fields: List[int]) -> np.array:
        state = state.copy()
    ...
    def _get_J(self, dt: float) -> np.array:
        ...
        J[StateIndex.X, StateIndex.YAW] = self._get_partial(
            -cp * sy, -cy * cr - sp * sy * sr, cy * sr - sp * sy * cr, dt
        )
        J[StateIndex.X, StateIndex.PITCH] = self._get_partial(
            -sp * cy, cp * cy * sr, cp * cy * cr, dt
        )
        J[StateIndex.X, StateIndex.ROLL] = self._get_partial(
            0, sp * cy * cr + sy * sr, sy * cr - sp * cy * sr, dt
        )
        J[StateIndex.Y, StateIndex.YAW] = self._get_partial(
            cp * cy, sp * cy * sr - sy * cr, sp * cy * sr + sy * sr, dt
        )
        J[StateIndex.Y, StateIndex.PITCH] = self._get_partial(
            -sp * sy, cp * sy * sr, cp * sy * cr, dt
        )
        J[StateIndex.Y, StateIndex.ROLL] = self._get_partial(
            0, sp * sy * cr - cy * sr, -cy * cr - sp * sy * sr, dt
        )
        J[StateIndex.Z, StateIndex.PITCH] = self._get_partial(
            -cp, -sp * sr, -sp * cr, dt
        )
        J[StateIndex.Z, StateIndex.ROLL] = self._get_partial(0, cp * sr, -cp * sr, dt)

        vyaw: float = self._state[StateIndex.VYAW]
        vpitch: float = self._state[StateIndex.VPITCH]

        J[StateIndex.YAW, StateIndex.PITCH] = (tp * sr / cp) * vpitch * dt + (
            tp * cr / cp
        ) * vyaw * dt
        J[StateIndex.YAW, StateIndex.ROLL] = (cr / cp) * vpitch * dt + (
            -sr / cp
        ) * vyaw * dt
        J[StateIndex.PITCH, StateIndex.ROLL] = (-sr) * vpitch * dt + (-cr) * vyaw * dt
        J[StateIndex.ROLL, StateIndex.PITCH] = (sr / (cp * cp)) * vpitch * dt + (
            cr / (cp * cp)
        ) * vyaw * dt
        J[StateIndex.ROLL, StateIndex.ROLL] = (
            1 + (tp * cr) * vpitch * dt + (-tp * sr) * vyaw * dt
        )

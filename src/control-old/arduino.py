...
class Arduino(Node):
    ...
    def __init__(self):
        ...
        self._enable_srv = self.create_service(
            MotorEnable, "motor_enable", self.enable_callback
        )
    ...
    def listener_callback(self, thruster_number, msg):
        ...
        if (
            False == self._allow_thrust
        ):  # nullify all thurst commands if flag is not enabled
    ...
    def enable_callback(self, request, response):
        ...
        if request.enable == True:
            ...
            self.get_logger().info(
                "Enable callback - Motors disabled and zero thrust sent"
            )
        return response
